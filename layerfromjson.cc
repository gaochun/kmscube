...
bool parseLayersFromJson(const char* jsonPath, LAYER_PARAMETERS& parameters)
{
  json_object *jso = NULL;
  jso = json_object_from_file(jsonPath);
  if (jso == NULL) {
    return false;
  }
  ...
  for(int i = 0; i < len; i++) {
    ...
    struct json_object* object = (struct json_object*)array_list_get_idx(array, i);
